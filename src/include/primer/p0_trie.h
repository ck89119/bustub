...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    auto node_ptr = (*cur).release();
    (*cur).reset(new TrieNodeWithValue<T>(std::move(*node_ptr), value));
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    latch_.WLock();
    auto status = RemoveInternal(key, 0, &root_);
    latch_.WUnlock();
    return status;
  }

  bool RemoveInternal(const std::string &key, uint32_t index, std::unique_ptr<TrieNode> *cur_node) {
    if (index >= key.size()) {
      auto status = (*cur_node)->IsEndNode();
      (*cur_node)->SetEndNode(false);
      return status;
    }

    if (!(*cur_node)->HasChild(key[index])) {
      return false;
    }

    auto next_node = (*cur_node)->GetChildNode(key[index]);
    auto status = RemoveInternal(key, index + 1, next_node);
    // remove node
    if ((!(*next_node)->HasChildren() && !(*next_node)->IsEndNode())) {
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    auto terminal_node = dynamic_cast<TrieNodeWithValue<T> *>((*cur).get());
